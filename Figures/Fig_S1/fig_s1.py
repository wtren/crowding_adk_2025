...
labs = ['dilute', 'inert', '$\epsilon_{attr}$=0.1', '$\epsilon_{attr}$=0.2', '$\epsilon_{attr}$=0.3']
num1 = 5
...
for j in range(3):
    for i in range(num1):
        ...
        if(j == 0):
            axs[j].plot(xi, yi_fit, lw=1.5, color=colors[i], label=labs[i])
        else:
            axs[j].plot(xi, yi_fit, lw=1.5, color=colors[i])
        axs[j].scatter(xi, yi, s=12, color=colors[i])

axs.format(xlabel='[ATP] $(\mathrm{\mu M})$')
axs.format(ylabel='Turnover rate (ms$^{-1}$)')
axs.format(grid=False)
# axs.legend(ncol=1, frameon=False)
# fig.legend(ncol=2, frameon=False, loc='b', fontsize='large')
axs[0].legend(ncol=1, frameon=False, fontsize='large')
...
fig.savefig("fig_s1.png", dpi=600)
fig.savefig("fig_s1.pdf")
fig.savefig("fig_s1.svg")
uplt.show()
